...
def run(raw_data):
    ...
    data = pd.concat([pd.DataFrame(data['Symptoms'].apply(lambda x: ' '.join(map(str,x))), columns=['Symptoms']),
                          pd.DataFrame(['']*len(data), columns=['ProductNr'])], axis=1)
    data_tr = model.pipe.transform(data)
    
    X = data_tr[[ col for col in data_tr.columns if col.startswith('feat')]]
    
    # make prediction
    y = model.predict(X)
    y = data_tr[[ col for col in y.columns if col.startswith('target')]]
    
    y_decoded = [
        [ 
            pipe.transformer_list[1][1].named_steps['target_encode'].col_cats[0][i] 
